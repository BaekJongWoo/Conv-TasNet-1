...
def main(argv):
    ...
    num_portions = (num_samples - param.overlap) // (param.K *
                                                     (param.L - param.overlap))

    # num_samples_output = num_portions * param.THat * (param.L - param.overlap)
    num_samples_output = num_portions * param.K * (param.L - param.overlap)
    ...
    model_input = np.zeros((num_portions, param.K, param.L))
    ...
    for i in range(num_portions):
        for j in range(param.K):
            begin = (i * param.K + j) * (param.L - param.overlap)
            end = begin + param.L
            model_input[i][j] = audio[begin:end]
