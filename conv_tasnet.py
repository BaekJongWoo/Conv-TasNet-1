...
class ConvTasNetEncoder(tf.keras.layers.Layer):
    ...
    Attributes:
        ...
        activation (str, optional): Nonlinear activation function for conv1d_U
...
class ConvTasNetSeparator(tf.keras.layers.Layer):
    ...
    Attributes:
        param (ConvTasNetParam): Hyperparameters
        normalization (gLN | cLN): Causality-depended layer normalization
        input_conv1x1 (keras.layers.Conv1D): 1x1 convolution layer
        TCN (TemporalConvNet): Dilated-TCN (main process)
        prelu (keras.layers.PReLU): Layer for the PReLU activation (with training parameter 'alpha')
        output_conv1x1 (keras.layers.Conv1D): 1x1 convolution layer (with sigmoid activation)
    """

    __slots__ = ('param' 'normalization', 'input_conv1x1',
                 'TCN', 'prelu', 'output_conv1x1', 'softmax')
    ...
    def call(self, mixture_weights):
        ...
        # [T_hat x B] -> [T_hat x Sc]
        tcn_outputs = self.TCN(tcn_inputs)
        # [T_hat x Sc] -> [T_hat x Sc]
        tcn_outputs = self.prelu(tcn_outputs)
        # [T_hat x Sc] -> [T_hat x CN] (with sigmoid activation)
...
class ConvTasNetDecoder(tf.keras.layers.Layer):
    ...
    Attributes:
        param (ConvTasNetParam): Hyperparameters
        multiply (keras.layers.Multiply): Layer for the elementwise multiplication
        conv1d_T (keras.layers.Conv1D): 1-D transpose convolution layer
        permute (keras.layers.Permute): [T_hat x C x L] -> [C x T_hat x L]
    """

    __slots__ = ('param', 'multiply', 'conv1d_T', 'permute')

    def __init__(self, param: ConvTasNetParam, **kwargs):
        super(ConvTasNetDecoder, self).__init__(**kwargs)
        self.param = param
        self.multiply = tf.keras.layers.Multiply()
        self.conv1d_T = tf.keras.layers.Conv1D(filters=self.param.L,
                                               kernel_size=1,
                                               use_bias=False)
    ...
    def call(self, mixture_weights, estimated_masks):
        ...
        estimated_sources = self.conv1d_T(estimated_weights)
        # [T_hat x C x L] -> [C x T_hat x L]
...
class ConvTasNet(tf.keras.Model):
    ...
    Attributes:
        ...
        encoder_activation (std, optional): Nonlinear activation function for conv1d_U
